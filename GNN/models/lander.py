...
class LANDER(nn.Module):
    def __init__(self, feature_dim, nhid, num_conv=4, dropout=0,
        ...
        output_dim = (nhid, nhid, nhid, nhid_half)


        self.nnconv = MPNNGNN(node_in_feats=self.feature_dim,
                           node_out_feats=128,
                           edge_in_feats=5,
                           edge_hidden_feats=5,
        ...
        self.conv.append(GraphConv(128, nhid, dropout, use_GAT, K))
        for i in range(1, num_conv):
            self.conv.append(GraphConv(nhid, output_dim[i], dropout, use_GAT, K))
    ...
    def forward(self, bipartites):
        if isinstance(bipartites, dgl.DGLGraph):
            ...
            neighbor_xe = bipartites[0].edata['efeat']
            neighbor_x = self.nnconv(bipartites[0],neighbor_x,neighbor_xe)
            ...
            neighbor_xe_src = bipartites[0].edata['efeat']
            center_xe_src = bipartites[1].edata['efeat']
            if self.use_cluster_feat:
                neighbor_x_src = torch.cat([bipartites[0].srcdata['features'], bipartites[0].srcdata['cluster_features']], axis=1)
                #neighbor_x_dst = torch.cat([bipartites[0].dstdata['features'], bipartites[0].dstdata['cluster_features']], axis=1)
                #neighbor_x_src = self.nnconv(bipartites[0],neighbor_x_src,neighbor_xe_src)
                center_x_src = torch.cat([bipartites[1].srcdata['features'], bipartites[1].srcdata['cluster_features']], axis=1)
                #center_x_dst = torch.cat([bipartites[1].dstdata['features'], bipartites[1].dstdata['cluster_features']], axis=1)
            else:
                neighbor_x_src = bipartites[0].srcdata['features']
                #neighbor_x_dst = bipartites[0].dstdata['features']
                center_x_src = bipartites[1].srcdata['features']
                #center_x_dst = bipartites[1].dstdata['features']
                #print("neighbor_x_src.shape:",neighbor_x_src)
                #print("neighbor_xe_src.shape:",neighbor_xe_src)
            #neighbor_x_src = self.nnconv(bipartites[0],(neighbor_x_src,neighbor_x_dst),neighbor_xe_src)
            #center_x_src = self.nnconv(bipartites[1],(center_x_src,center_x_dst),center_xe_src)

            neighbor_x_src = self.nnconv(bipartites[0],neighbor_x_src,neighbor_xe_src)
            center_x_src = self.nnconv(bipartites[1],center_x_src,center_xe_src)           
    ...
    def compute_loss(self, bipartite):
        ...
        #print("pred_den.shape:",pred_den.shape)
        #print("bipartite.dstdata['density'].shape:",bipartite.dstdata['density'].shape)
        ...
        if mask_conn.sum() > 0:
            #print("log_softmax(input, 1)bipartite.edata['pred_conn'][mask_conn]:", F.log_softmax(bipartite.edata['pred_conn'][mask_conn], 1))
            #print("labels_conn[mask_conn]:", labels_conn[mask_conn])
            loss_conn = self.loss_conn(bipartite.edata['pred_conn'][mask_conn], labels_conn[mask_conn])

            loss = loss_den + loss_conn
            #print("loss_den.item():",loss_den.item())
